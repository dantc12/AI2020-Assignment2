...
class AgentState:
    ...
    def state_terminate(self):
        if not self.curr_location.is_shelter():
            self.state_score -= (self.p_carrying + self.k_value)
        self.is_terminated = True

    def search_state_traverse(self, e, dest_v=None):
        if not dest_v:
        ...
        if e.weight == 1:
            ...
            self.curr_location = e
            self.is_traversing = True
            self.traverse_dest = dest_v
            self.traverse_timer = e.weight - 1
        self.time += 1

    def state_traverse_old(self, e, dest_v=None):
        if not dest_v:
            if self.curr_location == e.vertex_1:
                dest_v = e.vertex_2
            else:
    ...
    def search_state_traverse_update(self):
        self.time += 1
        self.traverse_timer -= 1
        if self.traverse_timer == 0:
            self.is_traversing = False
            self.curr_location = self.traverse_dest
            self.traverse_dest = None
    ...
    def state_pickup_loadoff_update(self):
        if not self.is_traversing:
            ...
            # Load off
                ...
                self.state_score += self.p_carrying
                self.p_carrying = 0

    def state_hurricane_check(self):
        location = self.curr_location
        if isinstance(location, Edge):
            if self.time > location.vertex_1.deadline or self.time > location.vertex_2.deadline:
                # self.state_score -= (self.k_value + self.p_carrying)
                return True
            else:
                return False
        else:
            if self.time > location.deadline:
                # self.state_score -= (self.k_value + self.p_carrying)
                return True
            else:
                return False
    ...
    def state_v_people_update(self):
        if isinstance(self.curr_location, Edge):
            a_vertex = self.curr_location.vertex_1
        else:
            a_vertex = self.curr_location
        for v in a_vertex.graph.vertices:
